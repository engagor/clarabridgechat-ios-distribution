...
@implementation CLBSTPFormTextField

@synthesize placeholderColor = _placeholderColor;

- (void)setFormDelegate:(id<CLBSTPFormTextFieldDelegate>)formDelegate {
...
- (NSAttributedString *)attributedStringForString:(NSString *)string attributes:(NSDictionary *)attributes {
    ...
    if (self.formatsCardNumbers && [CLBSTPCardValidator stringIsNumeric:string]) {
        NSArray *cardSpacing;
        CLBSTPCardBrand currentBrand = [CLBSTPCardValidator brandForNumber:attributedString.string];
        if (currentBrand == CLBSTPCardBrandAmex) {
            cardSpacing = @[@3, @9];
        } else {
            cardSpacing = @[@3, @7, @11];
        }
