...
#import "CLBSTPFormEncodable.h"

/**
 *  Representation of a user's credit card details. You can assemble these with information that your user enters and
 *  then create Stripe tokens with them using an CLBSTPAPIClient. @see https://stripe.com/docs/api#cards
 */
@interface CLBSTPCardParams : NSObject<CLBSTPFormEncodable>
 ...
 *  @deprecated use CLBSTPCardValidator instead.
 */
- (BOOL)validateNumber:(__nullable id * __nullable )ioValue
                 error:(NSError * __nullable * __nullable )outError __attribute__((deprecated("Use CLBSTPCardValidator instead.")));
 ...
 *  @deprecated use CLBSTPCardValidator instead.
 */
- (BOOL)validateCardReturningError:(NSError * __nullable * __nullable)outError __attribute__((deprecated("Use CLBSTPCardValidator instead.")));
