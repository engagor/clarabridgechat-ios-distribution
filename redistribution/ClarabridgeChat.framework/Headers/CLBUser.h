 ...
 *
 *  @discussion Unlike `userId`, this property is set automatically by ClarabridgeChat, and is not configurable. This is analogous to `appUser._id` in the REST API response. A user is assigned an `appUserId` once they start a conversation or when an existing user is logged in by calling +login:jwt:completionHandler:.
 *
 *  @see ClarabridgeChat
 */
